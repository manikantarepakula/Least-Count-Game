...
#screen-landing.screen.active, #screen-lobby.screen.active {
...
.hero-subtitle { color: #9fd8b8; font-size: 12px; margin: 4px 0 0; }
...
.landing-hero-glow {
  ...
  background: radial-gradient(circle, rgba(var(--red-rgb),0.35), rgba(var(--red-rgb),0) 68%);
  filter: blur(2px);
}
.landing-hero-cards {
...
.landing-wordmark {
  /* Cinzel -- Roman-inscription capitals, a regal/imperial display face --
     reserved exclusively for this one wordmark, never used for body text or
  ...
  font-size: 40px; letter-spacing: 1.5px; font-weight: 800; margin: 0;
  line-height: 1.1;
  background: linear-gradient(180deg, #ffffff 0%, #f2d9d9 55%, var(--red) 100%);
  -webkit-background-clip: text; background-clip: text; color: transparent;
  text-shadow: 0 6px 24px rgba(var(--red-rgb),0.25);
}
...
#screen-landing.screen.active::before,
#screen-lobby.screen.active::before {
  content: ''; position: absolute; inset: 0; z-index: 0; pointer-events: none;
  background:
    radial-gradient(circle at 50% 8%, rgba(var(--red-rgb),0.22), transparent 55%),
    repeating-linear-gradient(135deg, rgba(255,255,255,0.02) 0 2px, transparent 2px 34px);
}
#screen-landing .landing-topbar,
#screen-landing .landing-body,
#screen-landing .landing-footer-nav,
#screen-lobby .landing-topbar,
#screen-lobby .landing-body { position: relative; z-index: 1; }
...
/* Lobby's topbar only ever has the wordmark in it (no avatar/icons -- those
   are landing-only) -- centered rather than space-between, which would
   otherwise shove a single child over to one side. */
#screen-lobby .landing-topbar { justify-content: center; }
...
.landing-wordmark-sm { font-size: 18px; letter-spacing: 1.5px; }
...
.landing-toast-error {
  background: rgba(var(--red-rgb),0.14); border: 1px solid var(--red); color: #ff9d99;
...
.mode-block h3 { margin: 0 0 4px; font-size: 15px; color: var(--red); }
...
#screen-landing .mode-block button.primary,
#screen-lobby .mode-block button.primary {
...
#screen-lobby .mode-block .hint { color: #9a9aa2; }
#screen-lobby .mode-block .error { color: #ff8a80; }
...
#screen-lobby .room-code-display {
  background: transparent; color: var(--red); font-size: 38px; letter-spacing: 8px; text-align: center;
...
#screen-lobby .player-list .host-tag { color: var(--red); }
#screen-lobby .player-list .status { color: #9fd8b8; }
...
.footer-nav-item.active { color: var(--red); }
...
@media (min-width: 768px) {
  #screen-landing .landing-body,
  #screen-lobby .landing-body { max-width: 560px; padding: 20px 24px 14px; gap: 18px; }
...
#screen-landing .landing-form input[type=text]:focus { border-color: var(--red); }
#screen-landing .landing-form button.primary {
  background: linear-gradient(135deg, var(--red), var(--red-dark)); color: #fff;
  box-shadow: var(--shadow-md);
}
...
#screen-queue-waiting.screen.active { background: var(--jade-bg-dark); }
...
.landing-form label { color: #9fd8b8; }
.landing-form input[type=text] {
  background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.25); color: #fff;
}
.landing-form input[type=text]::placeholder { color: rgba(255,255,255,0.4); }
.landing-form input[type=text]:focus { border-color: var(--gold); }
.landing-form button.primary { background: var(--gold); color: #1a1a1a; }
.landing-form button.secondary {
  background: transparent; border: 1.5px solid var(--gold); color: var(--gold);
}
.landing-form .divider { color: #6ba889; }
.landing-form .hint { color: #9fd8b8; }
.landing-form .error { color: #ff8a80; }
.landing-form h3 {
...
.landing-form .room-code-display {
  background: transparent; color: var(--gold-bright); font-size: 38px; letter-spacing: 8px;
...
.landing-form .player-list .status { color: #6ba889; }
.landing-form .leave-btn { border-color: #c0392b; color: #e0736a; }
...
.lobby-maxscore-row label { color: #9fd8b8; }

.solo-panel { margin: 12px 0; }
.solo-panel.hidden { display: none; }
.solo-panel label { color: #9fd8b8; }
...
.dd-toggle[aria-expanded="true"] { border-color: var(--gold); }
...
.signin-status { color: #9fd8b8; font-size: 12px; flex: 1; }
.signin-row button.tiny { margin: 0; white-space: nowrap; }
.icon-btn {
  width: auto; flex: 0 0 auto; padding: 6px 9px; font-size: 15px; line-height: 1;
...
button.primary { background: linear-gradient(135deg, var(--red), var(--red-dark)); color: #fff; box-shadow: var(--shadow-md); }
button.secondary { background: transparent; border: 1.5px solid rgba(255,255,255,0.35); color: var(--text-light); }
button.danger { background: var(--red-dark); color: white; margin-top: 8px; }
button.declare { background: var(--gold); color: #1a1a1a; }
...
button.tiny { width: auto; padding: 8px 14px; font-size: 13px; }
button.leave-btn { background: transparent; color: var(--red); border: 1px solid var(--red); margin-top: 10px; }
...
.roomcode-feedback {
  min-height: 15px; color: #9fd8b8; font-weight: 600; margin: 2px 0 0; text-align: center;
}
...
.game-top {
  ...
  font-size: 13px; padding: 3px 4px; flex-shrink: 0;
}
.game-round { font-size: 12px; color: var(--text-muted); padding-left: 6px; letter-spacing: 0.02em; }
...
.table-felt {
  ...
  background: radial-gradient(circle, var(--jade-bg), var(--jade-bg-dark) 75%);
  border-radius: 50%;
...
.table-center .card { width: 58px; height: 82px; }
.table-center .card .card-corner { display: none; }
.table-center .card .center-rank { font-size: 20px; }
.table-center .card .center-suit { font-size: 26px; }
#joker-indicator { display: flex; }

.seat {
  position: absolute; transform: translate(-50%, -50%);
  display: flex; flex-direction: column; align-items: center; justify-content: center;
  gap: 3px; z-index: 3; width: 96px; text-align: center;
}
.seat-chip {
  background: rgba(255,255,255,0.14); border-radius: var(--radius-sm); padding: 5px 9px;
  font-size: 11.5px; text-align: center; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
...
.player-action-btn.report { color: #ff8a80; }
...
.seat-meta { font-size: 10px; opacity: 0.85; display: block; }
...
.seat-reveal-box {
  ...
  padding: 2px 3px;
  /* Slightly darker than .seat-chip's translucent white so the white card
     faces stay legible against it wherever it lands on the felt. */
  background: rgba(6,26,20,0.94);
  border: 1px solid var(--jade-border); border-radius: var(--radius-sm);
}
.seat-reveal-box.above { top: auto; bottom: calc(100% + 4px); }
...
.card.joker { color: #8b1a1a; }
.joker-letters {
  font-size: 7px; font-weight: 700; line-height: 1.15; color: #8b1a1a;
...
.chain-flash {
  ...
  background: linear-gradient(90deg, #c0392b, #e67e22); color: #fff;
...
.hand {
  ...
  flex: 0 0 auto; height: 148px; overflow-y: auto;
}

.action-bar { display: flex; gap: 8px; padding: 4px; flex-shrink: 0; }
.action-bar button { padding: 10px; font-size: 14px; }
...
.join-request-row .jr-admit { background: #1e7a4a; color: #fff; }
...
.scorecard-out { color: #ff8a80; font-size: 9px; font-weight: 700; margin-left: 3px; }
...
.round-result-note { text-align: center; font-size: 12.5px; color: #ff8a80; margin-bottom: 8px; }
.podium {
...
.chat-fab {
  position: fixed;
  /* Offset above the native banner (0 when none is showing) so the fab
     never ends up sitting half-underneath it -- this now matters on the
     game screen too, since the banner shows there as well. */
  bottom: calc(16px + var(--ad-safe-bottom, 0px));
  right: 16px; width: 52px; height: 52px; border-radius: 50%;
  background: linear-gradient(135deg, var(--red), var(--red-dark)); color: white; display: flex; align-items: center; justify-content: center;
  box-shadow: var(--shadow-md); cursor: pointer; z-index: 20;
}
.chat-fab .icon { width: 24px; height: 24px; margin: 0; }
...
/* Left at 28px. The density win came from dropping the per-message
   timestamp line and grouping consecutive messages, not from the avatar --
   shrinking this further would cost legibility for a few pixels. */
.chat-avatar {
...
.chat-msg.me .chat-name { color: var(--red); }
...
/* Lines up under the name when the avatar is omitted: 28px avatar + 9px gap. */
.chat-msg.grouped .chat-body { padding-left: 37px; }
...
.group-chat-msg.grouped { padding-top: 2px; }
...
.segmented .seg.active {
  background: rgba(229,37,44,0.16); border-color: rgba(229,37,44,0.5); color: var(--text-light);
}
...
.reveal-banner-title.wrong { color: #ff6b6b; }
.reveal-banner-sub {
  margin-top: 4px; font-size: 13px; font-weight: 700; color: var(--text-light);
  text-shadow: 0 2px 6px rgba(0,0,0,0.7);
}
.reveal-banner-sub.penalty { color: #ff6b6b; }

/* Outcome states on the seat boxes. .declared (gold) already existed for
   "this person called it"; these say whether it worked. */
.seat-reveal-box.winner  { border: 2px solid #4ade80; }
.seat-reveal-box.penalty { border: 2px solid #ff6b6b; }
.seat-reveal-box.penalty .seat-reveal-tag { color: #ff6b6b; }
.seat-reveal-box.winner  .seat-reveal-tag { color: #4ade80; }
...
.seat-reveal-score.zero { color: #4ade80; }
.seat-reveal-score.penalty { color: #ff6b6b; }
...
.gm-dot.playing { background: var(--red); box-shadow: 0 0 0 3px rgba(229,37,44,0.22); }
.gm-dot.table   { background: var(--gold-bright); box-shadow: 0 0 0 3px rgba(242,193,78,0.20); }
.gm-dot.online  { background: #4ade80; }
...
.group-chat-msg {
  ...
  background: var(--jade-panel-alt); font-size: 14px;
...
.group-chat-msg.mine { background: rgba(46,160,120,0.18); }
...
#btn-discard, #btn-declare { min-height: 44px; }
...
#btn-leave-round-result:active, #btn-leave-gameover:active { color: var(--red); }
